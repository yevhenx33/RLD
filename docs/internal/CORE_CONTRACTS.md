# RLD Core Contracts: Functionality & Test Coverage
...
| --- | -------------------------------------------------------------------------------------------- | ------------------------------------------------ |
| 1   | `normalizationFactor ≥ 1e18`                                                                 | `applyFunding()` (compounding only increases NF) |
| 2   | `totalDebt = Σ debtPrincipal` across all positions                                           | `modifyPosition()` delta accounting              |
| 3   | `wRLP.totalSupply() == totalDebt`                                                            | `mint`/`burn` 1:1 with deltaDebt                 |
| 4   | `collateral / (trueDebt × markPrice) ≥ maintenanceMargin` for all non-liquidatable positions | `isSolvent()` guard in `modifyPosition()`        |
| 5   | `MarketId` is unique per `(collateral, underlying, pool)` triple                             | `createMarket()` duplicate check                 |
| 6   | `PositionToken.owner() == address(core)` after market creation                               | Transferred by factory in `createMarket()`       |
| 7   | `debtCap == 0` behaves as unlimited                                                          | Sentinel handled in `modifyPosition()`           |
