...
const RLDPerformanceChart = ({
  ...
  const requestWindowRef = useRef({ older: 0, newer: 0 });

  const prevDataMeta = useRef({ startTs: 0, length: 0 });

  const requestDataWindow = React.useCallback((direction) => {
    if (!onRequestDataWindow) return;
    const now = Date.now();
    if (now - requestWindowRef.current[direction] < 600) return;
    requestWindowRef.current[direction] = now;
    onRequestDataWindow(direction);
  }, [onRequestDataWindow]);
  ...
  }, [data, requestDataWindow]); // Only re-bind if data or pagination callback changes
        ...
        >
          ...
          {areas.map((area, index) => (
            <Area
              ...
              fill={area.noFill ? "none" : `url(#gradient-${area.key})`}
              name={area.name}
