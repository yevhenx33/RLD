...
export default function OpenTradeModal({
  ...
  const lockedRate = (spread || 0) * lev;
  const estReturn = cap * (lockedRate / 100) * (days / 365);
      ...
      <div className="relative w-full max-w-md bg-[#080808] border border-white/10 shadow-2xl flex flex-col animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="p-6 flex flex-col gap-0">
          ...
          <div className="border border-white/10 bg-white/[0.02] p-4">
            <div className="text-[10px] text-gray-500 uppercase tracking-widest mb-2">You Deposit</div>
            <div className="flex items-baseline justify-between">
              <span className="text-2xl font-light text-white tracking-tight">
                {cap.toLocaleString()}
              </span>
              <span className="text-xs text-gray-500 uppercase tracking-widest">{selectedToken}</span>
            </div>
          </div>
          ...
          <div className="border border-pink-500/30 bg-pink-500/[0.04] p-5">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-[10px] text-pink-400 uppercase tracking-widest">Fixed At</span>
            </div>
            <div className="flex items-baseline justify-between">
              <span className="text-4xl font-light tracking-tight text-pink-400">
                {lockedRate >= 0 ? "+" : ""}{formatNum(lockedRate, 2)}%
              </span>
              <span className="text-xs text-gray-500 uppercase tracking-widest">{days}D · {lev}×</span>
            </div>
          </div>
        ...
        <div className="px-6 pb-5">
          ...
          >
            {executing ? (<><Loader2 size={14} className="animate-spin" />{executionStep || "Processing..."}</>) : "Open Position"}
          </button>
        </div>
