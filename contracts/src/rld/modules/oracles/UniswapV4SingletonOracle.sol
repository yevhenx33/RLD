...
/// @dev F-04 FIX: Only the owner (deployer/factory) can register pools.
contract UniswapV4SingletonOracle is ISpotOracle, Owned {
    struct PoolSettings {
        PoolKey key;
        PoolId poolId;
        IJTM twamm;
    ...
    /// @notice Registers a V4 pool for mark price queries.
    /// @dev F-04 FIX: Only owner can register to prevent unauthorized oracle manipulation.
    ...
    function getSpotPrice(
        ...
        int56[] memory tickCumulatives = settings.twamm.observe(
            settings.poolId,
            secondsAgos
        );
        ...
        int24 arithmeticMeanTick = int24(
            tickCumulativesDelta / int56(uint56(settings.period))
        );

        if (
            tickCumulativesDelta < 0 &&
            (tickCumulativesDelta % int56(uint56(settings.period)) != 0)
        ) {
            arithmeticMeanTick--;
        }
        ...
        uint128 baseAmount = uint128(
            10 ** (zeroForOne ? decimals0 : decimals1)
        );
        uint256 quoteAmount = getQuoteAtTick(
            arithmeticMeanTick,
            baseAmount,
            collateralToken,
            underlyingToken,
