...
contract MintAndLPScript is Script {
    ...
    function run() external {
        ...
        (bool success,) = AAVE_POOL.call(
            abi.encodeWithSignature("supply(address,uint256,address,uint16)", USDC, COLLATERAL_AMOUNT, deployer, 0)
        );
        ...
        (bool createSuccess, bytes memory result) =
            brokerFactory.call(abi.encodeWithSignature("createBroker(bytes32)", salt));
        ...
        (bool mintSuccess,) = broker.call(
            abi.encodeWithSignature("modifyPosition(bytes32,int256,int256)", marketId, int256(0), int256(DEBT_AMOUNT))
        );
        ...
        (bool withdrawSuccess,) =
            broker.call(abi.encodeWithSignature("withdraw(address,uint256)", positionToken, wrlpForLp));
        ...
        (bool withdrawAusdcSuccess,) =
            broker.call(abi.encodeWithSignature("withdraw(address,uint256)", AUSDC, aUsdcForLp));
        ...
        (address currency0, address currency1) = positionToken < AUSDC ? (positionToken, AUSDC) : (AUSDC, positionToken);
