...
class FluidSourceTests(unittest.TestCase):
    ...
    def test_strict_price_support_classification(self):
        ...
        status, feeds, reason = source._support_for_symbol("reUSD")
        self.assertEqual(status, "ORACLE_SNAPSHOT_REQUIRED")
        self.assertIn("Fluid vault oracle", reason)

    def test_reusd_fluid_vault_oracle_registry(self):
        from analytics.fluid_registry import FLUID_VAULT_ORACLE_FEEDS

        feeds = FLUID_VAULT_ORACLE_FEEDS["REUSD"]
        self.assertEqual({feed["quote"] for feed in feeds}, {"USDC", "USDT", "GHO"})
        self.assertTrue(all(feed["oracle"].startswith("0x") for feed in feeds))

    def test_snapshot_required_asset_without_fresh_snapshot_is_not_written(self):
        ...
        self.assertEqual(source._write_snapshots(FakeClickHouse(), rows), 0)

    def test_rpc_call_many_reuses_prefetched_multicall_cache(self):
        from analytics.scripts.backfill_fluid_product_snapshots import MulticallRequest, RpcClient, RpcResult, SELECTORS, normalize_address

        rpc = RpcClient("http://127.0.0.1:1")
        token = "0x40d16fc0246ad3160ccc09b8d0d3a2cd28ae6c2f"
        key = (normalize_address(token), SELECTORS["decimals"].lower(), hex(100))
        rpc._cache[key] = RpcResult(True, "0x" + word(18), "", "")

        result = rpc.call_many([MulticallRequest(token, SELECTORS["decimals"])], 100)

        self.assertEqual(len(result), 1)
        self.assertTrue(result[0].ok)
        self.assertEqual(rpc.rpc_calls, 0)
