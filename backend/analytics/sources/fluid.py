...
from ..fluid_registry import INTENTIONAL_UNPRICED_SYMBOLS, fluid_symbol_key
...
class FluidSource(BaseSource):
    ...
    def _support_for_symbol(self, symbol: str) -> tuple[str, tuple[str, ...], str]:
        intentional_reason = INTENTIONAL_UNPRICED_SYMBOLS.get(fluid_symbol_key(symbol))
        if intentional_reason:
            return "UNPRICED", (), intentional_reason
    ...
    def _write_snapshots(self, ch, decoded_rows: list[dict]) -> int:
        ...
        snapshot_price_frame = pd.DataFrame()
        try:
            ttl_sec = max(0, int(os.getenv("FLUID_ORACLE_SNAPSHOT_TTL_SEC", "21600") or "21600"))
            min_snapshot_ts = (pd.to_datetime(hourly["ts"].min()) - pd.Timedelta(seconds=ttl_sec)).strftime("%Y-%m-%d %H:%M:%S")
            max_snapshot_ts = pd.to_datetime(hourly["ts"].max()).strftime("%Y-%m-%d %H:%M:%S")
            snapshot_df = ch.query_df(f"""
                SELECT toStartOfHour(timestamp) AS ts, lower(subject) AS subject,
                       argMax(price_usd, tuple(timestamp, block_number)) AS price
                FROM oracle_snapshots FINAL
                WHERE source = 'FLUID' AND status = 'OK'
                  AND timestamp >= '{min_snapshot_ts}' AND timestamp <= '{max_snapshot_ts}'
                GROUP BY ts, subject ORDER BY ts, subject
                """)
            if not snapshot_df.empty:
                snapshot_price_frame = snapshot_df.pivot_table(index="ts", columns="subject", values="price", aggfunc="last").sort_index().ffill()
        except Exception as exc:
            log.warning("[%s] Failed to load Fluid oracle snapshots: %s", self.name, exc)
            snapshot_price_frame = pd.DataFrame()
        for token in hourly["token"].unique():
            ...
            elif support == "ORACLE_SNAPSHOT_REQUIRED" and not snapshot_price_frame.empty and str(token).lower() in snapshot_price_frame.columns:
                supported_tokens.add(str(token))
        ...
        if prices.empty and snapshot_price_frame.empty:
            return 0
        metrics = []
        for row in hourly.itertuples(index=False):
            ...
            if support == "CHAINLINK_SUPPORTED":
                ...
                if not snapshot_price_frame.empty and token_key in snapshot_price_frame.columns:
                    snapshot_rows_for_ts = snapshot_price_frame.loc[snapshot_price_frame.index <= ts]
                    if not snapshot_rows_for_ts.empty:
                        price_value = snapshot_rows_for_ts.iloc[-1].get(token_key)
