...
from analytics.fluid_registry import CHAINLINK_PROXY_FEEDS, FLUID_VAULT_ORACLE_FEEDS, MULTICALL3, RATE_PROVIDER_FEEDS, SNAPSHOT_REQUIRED_SYMBOLS, fluid_symbol_key
...
SELECTORS = {
    ...
    "getExchangeRateOperate": selector("getExchangeRateOperate()"),
    "getExchangeRateLiquidate": selector("getExchangeRateLiquidate()"),
    "targetDecimals": selector("targetDecimals()"),
    "infoName": selector("infoName()"),
}
MULTICALL_AGGREGATE3_SELECTOR = selector("aggregate3((address,bool,bytes)[])")
...
class RpcClient:
    def __init__(self, rpc_url: str, timeout_sec: int = 60, retries: int = 2):
        ...
        self._id = 0
        self.batch_size = max(1, int(os.getenv("FLUID_MULTICALL_BATCH_SIZE", "250") or "250"))
        self.rpc_calls = 0
        self.multicall_batches = 0
        self.multicall_calls = 0
        self._cache: dict[tuple[str, str, str], RpcResult] = {}

    def _block_tag(self, block: str | int) -> str:
        return hex(int(block)) if isinstance(block, int) else str(block)

    def call(self, to: str, data: str, block: str | int = "latest") -> RpcResult:
        block_tag = self._block_tag(block)
        cache_key = (normalize_address(to), str(data).lower(), block_tag)
        ...
        for attempt in range(self.retries + 1):
            try:
                ...
                if item.get("error"):
                    ...
                    result = RpcResult(False, "0x", str(err.get("message") if isinstance(err, dict) else err)[:500], str(err_data or ""))
                    self._cache[cache_key] = result
                    return result
                result = RpcResult(True, str(item.get("result") or "0x"), "", "")
                self._cache[cache_key] = result
                return result
        ...
        result = RpcResult(False, "0x", last_error, "")
        self._cache[cache_key] = result
        return result

    def call_many(self, requests_: list[MulticallRequest], block: str | int = "latest") -> list[RpcResult]:
        if not requests_:
            return []
        if len(requests_) == 1:
            req = requests_[0]
            return [self.call(req.to, req.data, block)]
        out: list[RpcResult] = []
        block_tag = self._block_tag(block)
        for start in range(0, len(requests_), self.batch_size):
            batch = requests_[start : start + self.batch_size]
            cached: list[RpcResult | None] = []
            misses: list[MulticallRequest] = []
            for req in batch:
                key = (normalize_address(req.to), str(req.data).lower(), block_tag)
                cached_result = self._cache.get(key)
                cached.append(cached_result)
                if cached_result is None:
                    misses.append(req)
            if not misses:
                out.extend([result for result in cached if result is not None])
                continue
            encoded_calls = [
                (normalize_address(req.to), bool(req.allow_failure), bytes.fromhex(req.data.removeprefix("0x")))
                for req in misses
            ]
            data = MULTICALL_AGGREGATE3_SELECTOR + abi_encode(["(address,bool,bytes)[]"], [encoded_calls]).hex()
            res = self.call(MULTICALL3, data, block)
            self.multicall_batches += 1
            self.multicall_calls += len(misses)
            missed_results: list[RpcResult] = []
            if not res.ok:
                missed_results = [self.call(req.to, req.data, block) for req in misses]
            else:
                try:
                    decoded = abi_decode(["(bool,bytes)[]"], bytes.fromhex(res.result[2:]))[0]
                    for req, (success, return_data) in zip(misses, decoded, strict=False):
                        raw = "0x" + bytes(return_data).hex()
                        result = RpcResult(bool(success), raw, "" if success else "multicall subcall failed", raw if not success else "")
                        self._cache[(normalize_address(req.to), str(req.data).lower(), block_tag)] = result
                        missed_results.append(result)
                except Exception as exc:
                    err = str(exc)[:500]
                    missed_results = [RpcResult(False, "0x", err, "") for _ in misses]
                    for req, result in zip(misses, missed_results, strict=False):
                        self._cache[(normalize_address(req.to), str(req.data).lower(), block_tag)] = result
            missed_iter = iter(missed_results)
            for cached_result in cached:
                out.append(cached_result if cached_result is not None else next(missed_iter))
        return out

    def prefetch(self, requests_: list[MulticallRequest], block: str | int = "latest") -> None:
        self.call_many(requests_, block)
...
def latest_oracle_prices(ch) -> dict[str, float]:
    ttl_sec = max(0, int(os.getenv("FLUID_ORACLE_SNAPSHOT_TTL_SEC", "21600") or "21600"))
    freshness_filter = ""
    if ttl_sec > 0:
        freshness_filter = f"AND timestamp >= now() - INTERVAL {ttl_sec} SECOND"
    try:
        rows = ch.query(
            f"""
...
def resolve_quote_price_usd(symbol: str, prices: PriceContext) -> tuple[float, str]:
    price, status = resolve_fluid_feed_price(symbol, prices.feed_prices)
    if price > 0:
        return float(price), status or "CHAINLINK"
    token = token_address_for_symbol(symbol)
    if token:
        snap = prices.oracle_prices.get(token, 0.0)
        if snap > 0:
            return float(snap), "ORACLE_SNAPSHOT"
        reserve = prices.reserve_prices.get(token, 0.0)
        if reserve > 0:
            return float(reserve), "FLUID_RESERVE_PRICE"
    return 0.0, "MISSING_QUOTE_PRICE"
...
def discover_indexed_contracts(rpc: RpcClient, factory: str, total_sig: str, getter_sig: str, block: str | int, max_count: int | None = None) -> tuple[list[str], str]:
    ...
    requests_ = [MulticallRequest(factory, SELECTORS[getter_sig] + word_uint(idx)) for idx in range(1, int(total) + 1)]
    contracts: list[str] = []
    errors: list[str] = []
    for idx, res in enumerate(rpc.call_many(requests_, block), start=1):
        if not res.ok:
            errors.append(f"{idx}:{res.error}")
            continue
        try:
            contract = normalize_address(decode_single(res.result, "address"))
            if contract and contract != ZERO_ADDRESS:
                contracts.append(contract)
        except Exception as exc:
            errors.append(f"{idx}:{str(exc)[:200]}")
...
def _decode_uint_result(res: RpcResult) -> tuple[int | None, str]:
    if not res.ok:
        return None, res.error
    try:
        return int(decode_single(res.result, "uint256")), ""
...
def _decode_address_result(res: RpcResult) -> tuple[str, str]:
    if not res.ok:
        return "", res.error
    try:
        return normalize_address(decode_single(res.result, "address")), ""
...
def _decode_latest_answer_result(res: RpcResult) -> tuple[int | None, str]:
    if not res.ok:
        return None, res.error
    try:
        return int(abi_decode(["int256"], bytes.fromhex(res.result[2:]))[0]), ""
...
def _decode_latest_round_result(res: RpcResult) -> tuple[tuple | None, str]:
    if not res.ok:
        return None, res.error
    try:
        return abi_decode(["(uint80,int256,uint256,uint256,uint80)"], bytes.fromhex(res.result[2:]))[0], ""
...
def _decode_string_result(res: RpcResult) -> tuple[str, str]:
    if not res.ok:
        return "", res.error
    try:
        return str(decode_single(res.result, "string")), ""
...
def _quote_to_usd(rate: float, quote: str, prices: PriceContext) -> tuple[float, str]:
    quote = str(quote or "USD").upper()
    if quote == "USD":
        return float(rate), "USD"
    quote_price, quote_status = resolve_fluid_feed_price(quote, prices.feed_prices)
    if quote_price <= 0:
        return 0.0, f"missing {quote} / USD feed"
    return float(rate) * quote_price, quote_status


def _snapshot_fluid_vault_oracle_price(
    rpc: RpcClient,
    asset: str,
    symbol: str,
    ...
    timestamp: dt.datetime,
    prices: PriceContext,
) -> tuple[OracleSnapshot | None, str]:
    calls: list[MulticallRequest] = []
    for cfg in configs:
        oracle = str(cfg["oracle"])
        calls.extend([
            MulticallRequest(oracle, SELECTORS["getExchangeRateOperate"]),
            MulticallRequest(oracle, SELECTORS["getExchangeRateLiquidate"]),
            MulticallRequest(oracle, SELECTORS["targetDecimals"]),
            MulticallRequest(oracle, SELECTORS["infoName"]),
        ])
    results = rpc.call_many(calls, block_number)
    metadata_fallbacks: dict[int, tuple[int | None, str, str]] = {}
    metadata_calls: list[MulticallRequest] = []
    metadata_indexes: list[int] = []
    for idx, cfg in enumerate(configs):
        target_decimals, target_err = _decode_uint_result(results[idx * 4 + 2])
        info_name, _info_err = _decode_string_result(results[idx * 4 + 3])
        if target_decimals is None:
            oracle = str(cfg["oracle"])
            metadata_indexes.append(idx)
            metadata_calls.extend([
                MulticallRequest(oracle, SELECTORS["targetDecimals"]),
                MulticallRequest(oracle, SELECTORS["infoName"]),
            ])
        else:
            metadata_fallbacks[idx] = (int(target_decimals), info_name, "")
    if metadata_calls:
        latest_metadata = rpc.call_many(metadata_calls, "latest")
        for offset, idx in enumerate(metadata_indexes):
            target_decimals, target_err = _decode_uint_result(latest_metadata[offset * 2])
            info_name, _info_err = _decode_string_result(latest_metadata[offset * 2 + 1])
            metadata_fallbacks[idx] = (target_decimals, info_name, target_err)

    paths: list[dict[str, Any]] = []
    errors: list[str] = []
    for idx, cfg in enumerate(configs):
        operate, operate_err = _decode_uint_result(results[idx * 4])
        liquidate, liquidate_err = _decode_uint_result(results[idx * 4 + 1])
        target_decimals, info_name, target_err = metadata_fallbacks.get(idx, (None, "", "missing oracle metadata"))
        quote = str(cfg.get("quote") or "").upper()
        quote_price, quote_status = resolve_quote_price_usd(quote, prices)
        if not operate or target_decimals is None or quote_price <= 0:
            errors.append(f"{cfg.get('feed')}: {operate_err or target_err or quote_status or liquidate_err or 'missing oracle path'}")
            continue
        if liquidate and abs(float(operate) - float(liquidate)) / max(float(operate), 1.0) > 0.005:
            errors.append(f"{cfg.get('feed')}: operate/liquidate mismatch")
            continue
        quote_per_asset = float(operate) / float(10 ** int(target_decimals))
        price_usd = quote_per_asset * quote_price
        if price_usd <= 0:
            errors.append(f"{cfg.get('feed')}: non-positive USD price")
            continue
        paths.append({
            "oracle": normalize_address(str(cfg["oracle"])),
            "vault": normalize_address(str(cfg.get("vault") or "")),
            "feed": str(cfg.get("feed") or info_name),
            "quote": quote,
            "quoteStatus": quote_status,
            "quotePriceUsd": quote_price,
            "targetDecimals": int(target_decimals),
            "operateRaw": str(operate),
            "liquidateRaw": str(liquidate or 0),
            "quotePerAsset": quote_per_asset,
            "priceUsd": price_usd,
            "infoName": info_name,
        })
    if len(paths) < 2:
        return None, "; ".join(errors[:5]) or "fewer than two valid Fluid vault oracle paths"
    values = [float(path["priceUsd"]) for path in paths]
    price_usd = float(statistics.median(values))
    max_rel_diff = max(abs(value - price_usd) / max(price_usd, 1e-18) for value in values)
    if max_rel_diff > 0.005:
        return None, f"Fluid vault oracle paths disagree: max_rel_diff={max_rel_diff:.8f}; paths={json.dumps(paths, sort_keys=True)}"[:1000]
    meta = {"paths": paths, "medianPriceUsd": price_usd, "maxRelativeDiff": max_rel_diff}
    return OracleSnapshot(
        ...
        asset,
        paths[0]["oracle"],
        "median(getExchangeRateOperate)",
        block_number,
        timestamp,
        str(int(statistics.median([int(path["operateRaw"]) for path in paths]))),
        "median",
        price_usd,
        "OK",
...
def _snapshot_explicit_oracles_multicall(
    rpc: RpcClient,
    ch,
    ...
    timestamp: dt.datetime,
    prices: PriceContext,
) -> tuple[list[OracleSnapshot], set[str]]:
    ...
    for asset, symbol in rows:
        ...
        if feed_price > 0:
            ...
            unresolved.append((asset, symbol, fluid_symbol_key(symbol)))

    for asset, symbol, key in list(unresolved):
        configs = FLUID_VAULT_ORACLE_FEEDS.get(key)
        if not configs:
            continue
        snapshot, error = _snapshot_fluid_vault_oracle_price(rpc, asset, symbol, configs, block_number, timestamp, prices)
        if snapshot:
            snapshots.append(snapshot)
            prices.oracle_prices[asset] = float(snapshot.price_usd)
            resolved.add(asset)
        elif error:
            snapshots.append(OracleSnapshot(ETHEREUM_CHAIN_ID, "FLUID", "FLUID_VAULT_ORACLE", asset, str(configs[0].get("oracle") or asset), "median(getExchangeRateOperate)", block_number, timestamp, "0", "median", 0.0, "ERROR", error[:1000]))

    proxy_items = [(asset, symbol, key, CHAINLINK_PROXY_FEEDS[key]) for asset, symbol, key in unresolved if key in CHAINLINK_PROXY_FEEDS]
    proxy_calls: list[MulticallRequest] = []
    for _asset, _symbol, _key, cfg in proxy_items:
        proxy = str(cfg["proxy"])
        proxy_calls.extend([
            MulticallRequest(proxy, SELECTORS["decimals"]),
            MulticallRequest(proxy, SELECTORS["latestRoundData"]),
            MulticallRequest(proxy, SELECTORS["latestAnswer"]),
        ])
    proxy_results = rpc.call_many(proxy_calls, block_number)
    proxy_errors: dict[str, str] = {}
    for idx, (asset, _symbol, _key, cfg) in enumerate(proxy_items):
        decimals, dec_err = _decode_uint_result(proxy_results[idx * 3])
        round_data, round_err = _decode_latest_round_result(proxy_results[idx * 3 + 1])
        answer_data, answer_err = _decode_latest_answer_result(proxy_results[idx * 3 + 2])
        method = str(cfg.get("method") or "latestRoundData")
        answer = answer_data if method == "latestAnswer" else int(round_data[1]) if round_data else answer_data
        if method != "latestAnswer" and not round_data and answer is not None:
            method = "latestAnswer"
        if decimals is None or answer is None or int(answer) <= 0:
            proxy_errors[asset] = dec_err or round_err or answer_err or "Chainlink proxy read failed"
            continue
        rate = float(answer) / float(10 ** int(decimals))
        price_usd, quote_status = _quote_to_usd(rate, str(cfg.get("quote") or "USD"), prices)
        if price_usd <= 0:
            proxy_errors[asset] = quote_status
            continue
        meta = {
            "proxy": cfg["proxy"],
            "feed": cfg["feed"],
            "quote": str(cfg.get("quote") or "USD").upper(),
            "answer": str(answer),
            "decimals": int(decimals),
            "roundId": str(round_data[0]) if round_data else "",
            "updatedAt": int(round_data[3] or 0) if round_data else 0,
        ...
        snapshots.append(OracleSnapshot(ETHEREUM_CHAIN_ID, "FLUID", "CHAINLINK_PROXY", asset, cfg["proxy"], cfg["feed"], block_number, timestamp, str(answer), str(10 ** int(decimals)), float(price_usd), "OK", json.dumps(meta, sort_keys=True)))
        prices.oracle_prices[asset] = float(price_usd)
        resolved.add(asset)

    remaining = [(asset, symbol, key) for asset, symbol, key in unresolved if asset not in resolved]
    rate_items = [(asset, symbol, key, RATE_PROVIDER_FEEDS[key]) for asset, symbol, key in remaining if key in RATE_PROVIDER_FEEDS]
    rate_calls: list[MulticallRequest] = []
    for _asset, _symbol, _key, cfg in rate_items:
        contract = str(cfg["contract"])
        rate_calls.extend([MulticallRequest(contract, SELECTORS["decimals"]), MulticallRequest(contract, SELECTORS[str(cfg.get("method") or "getRate")])])
    rate_results = rpc.call_many(rate_calls, block_number)
    rate_errors: dict[str, str] = {}
    for idx, (asset, _symbol, _key, cfg) in enumerate(rate_items):
        decimals, dec_err = _decode_uint_result(rate_results[idx * 2])
        rate_raw, rate_err = _decode_uint_result(rate_results[idx * 2 + 1])
        if decimals is None or rate_raw is None or int(rate_raw) <= 0:
            rate_errors[asset] = dec_err or rate_err or "rate provider read failed"
            continue
        rate = float(rate_raw) / float(10 ** int(decimals))
        price_usd, quote_status = _quote_to_usd(rate, str(cfg.get("quote") or "USD"), prices)
        if price_usd <= 0:
            rate_errors[asset] = quote_status
            continue
        meta = {"contract": cfg["contract"], "feed": cfg["feed"], "quote": str(cfg.get("quote") or "USD").upper(), "rateRaw": str(rate_raw), "decimals": int(decimals), "method": str(cfg.get("method") or "getRate"), "quoteStatus": quote_status}
        snapshots.append(OracleSnapshot(ETHEREUM_CHAIN_ID, "FLUID", "RATE_PROVIDER", asset, cfg["contract"], cfg["feed"], block_number, timestamp, str(rate_raw), str(10 ** int(decimals)), float(price_usd), "OK", json.dumps(meta, sort_keys=True)))
        prices.oracle_prices[asset] = float(price_usd)
        resolved.add(asset)

    erc_items = [(asset, symbol, key) for asset, symbol, key in unresolved if asset not in resolved]
    probe_calls: list[MulticallRequest] = []
    for asset, _symbol, _key in erc_items:
        probe_calls.extend([MulticallRequest(asset, SELECTORS["decimals"]), MulticallRequest(asset, SELECTORS["asset"]), MulticallRequest(asset, SELECTORS["underlyingAsset"])])
    probe_results = rpc.call_many(probe_calls, block_number)
    convert_items: list[tuple[str, str, int, str]] = []
    erc_errors: dict[str, str] = {}
    for idx, (asset, _symbol, _key) in enumerate(erc_items):
        decimals, dec_err = _decode_uint_result(probe_results[idx * 3])
        underlying, asset_err = _decode_address_result(probe_results[idx * 3 + 1])
        if not underlying:
            underlying, asset_err = _decode_address_result(probe_results[idx * 3 + 2])
        if decimals is None or not underlying:
            erc_errors[asset] = asset_err or dec_err or proxy_errors.get(asset) or rate_errors.get(asset) or "asset()/convertToAssets() unsupported"
            continue
        convert_items.append((asset, underlying, int(decimals), SELECTORS["convertToAssets"] + word_uint(10 ** int(decimals))))
    convert_results = rpc.call_many([MulticallRequest(asset, data) for asset, _underlying, _decimals, data in convert_items], block_number)
    for idx, (asset, underlying, _decimals, _data) in enumerate(convert_items):
        assets, conv_err = _decode_uint_result(convert_results[idx])
        base = resolve_fluid_token_price(underlying, token_symbol(underlying), prices)
        if assets is None or base.pricing_status != "PRICED":
            erc_errors[asset] = conv_err or base.reason or proxy_errors.get(asset) or rate_errors.get(asset) or "missing underlying price"
            continue
    ...
    for asset, _symbol, _key in unresolved:
        if asset not in resolved:
            error = erc_errors.get(asset) or proxy_errors.get(asset) or rate_errors.get(asset) or "no supported Fluid oracle resolver"
            snapshots.append(OracleSnapshot(ETHEREUM_CHAIN_ID, "FLUID", "UNSUPPORTED", asset, asset, "multicall_probe", block_number, timestamp, "0", "0", 0.0, "ERROR", error[:1000]))
...
def snapshot_fluid_oracles(rpc: RpcClient, ch, block_number: int, timestamp: dt.datetime, prices: PriceContext) -> int:
    symbol_list = ", ".join("'" + symbol.replace("'", "''") + "'" for symbol in SNAPSHOT_REQUIRED_SYMBOLS)
    try:
        rows = ch.query(
            f"""
            ...
            WHERE oracle_support = 'ORACLE_SNAPSHOT_REQUIRED'
               OR upper(toString(symbol)) IN ({symbol_list})
            GROUP BY asset
            """
        ).result_rows
    except Exception:
        rows = []
    snapshots, _resolved = _snapshot_explicit_oracles_multicall(rpc, ch, [(str(asset), str(symbol or "")) for asset, symbol in rows], block_number, timestamp, prices)
    return insert_oracle_snapshots(ch, snapshots)


def prefetch_product_snapshot_calls(rpc: RpcClient, ftokens: list[str], vaults: list[str], dexes: list[str], block_number: int) -> None:
    calls: list[MulticallRequest] = []
    for token in ftokens:
        calls.extend([
            MulticallRequest(token, SELECTORS["symbol"]),
            MulticallRequest(token, SELECTORS["asset"]),
            MulticallRequest(token, SELECTORS["underlyingAsset"]),
            MulticallRequest(token, SELECTORS["decimals"]),
            MulticallRequest(token, SELECTORS["totalAssets"]),
            MulticallRequest(token, SELECTORS["totalSupply"]),
        ])
    for dex in dexes:
        calls.extend([
            MulticallRequest(dex, SELECTORS["constantsView"]),
            MulticallRequest(dex, SELECTORS["constantsView2"]),
            MulticallRequest(dex, SELECTORS["getPricesAndExchangePrices"]),
            MulticallRequest(dex, SELECTORS["readFromStorage"] + word_uint(DEX_TOTAL_SUPPLY_SHARES_SLOT)),
            MulticallRequest(dex, SELECTORS["readFromStorage"] + word_uint(DEX_TOTAL_BORROW_SHARES_SLOT)),
        ])
    for vault in vaults:
        calls.extend([
            MulticallRequest(vault, SELECTORS["TYPE"]),
            MulticallRequest(vault, SELECTORS["constantsView"]),
            MulticallRequest(vault, SELECTORS["readFromStorage"] + word_uint(0)),
            MulticallRequest(vault, SELECTORS["readFromStorage"] + word_uint(8)),
            MulticallRequest(FLUID_VAULT_RESOLVER, SELECTORS["getVaultEntireData"] + word_address(vault)),
        ])
    calls.append(MulticallRequest(FLUID_REVENUE_RESOLVER, SELECTORS["getRevenues"]))
    rpc.prefetch(calls, block_number)
...
def run(args) -> int:
    ...
    try:
        ...
        prefetch_product_snapshot_calls(rpc, ftokens, vaults, dexes, block_number)
        ...
        if args.dry_run:
            print(json.dumps({"block": block_number, "ftokens": len(ftokens), "vaults": len(vaults), "dexes": len(dexes), "snapshots": len(snapshot_rows), "components": len(component_rows), "oracleSnapshots": oracle_snapshots, "rpcCalls": rpc.rpc_calls, "multicallBatches": rpc.multicall_batches, "multicallCalls": rpc.multicall_calls, "errors": {"ftokens": ftoken_err, "vaults": vault_err, "dexes": dex_err}}, indent=2))
            return 0
        ...
        print(json.dumps({"block": block_number, "snapshots": len(snapshot_rows), "components": len(component_rows), "contracts": len(contract_rows), "oracleSnapshots": oracle_snapshots, "ftokens": len(ftokens), "vaults": len(vaults), "dexes": len(dexes), "rpcCalls": rpc.rpc_calls, "multicallBatches": rpc.multicall_batches, "multicallCalls": rpc.multicall_calls}, indent=2))
        return 0
    finally:
        ch.close()
