...
from analytics.fluid_full_coverage import ETHEREUM_CHAIN_ID, FLUID_LENDING_FACTORY, ensure_fluid_anchor_tables, ensure_fluid_full_coverage_tables
...
def _shared_price_asset(token: str) -> str:
    normalized = normalize_address(token)
    return WETH_ADDRESS if normalized == NATIVE_ETH_ADDRESS else normalized


def _price_usd(ch, token: str) -> float:
    asset = _shared_price_asset(token)
    if _table_exists(ch, "asset_price_latest"):
        rows = ch.query("""
            SELECT argMax(price_usd, timestamp)
            FROM asset_price_latest FINAL
            WHERE lower(asset_address) = %(asset)s
              AND status = 'OK'
              AND price_usd > 0
        """, parameters={"asset": asset.lower()}).result_rows
        if rows and rows[0][0]:
            return float(rows[0][0])
    if _table_exists(ch, "fluid_reserve_metrics"):
        rows = ch.query(
            "SELECT argMax(price_usd, timestamp) FROM fluid_reserve_metrics FINAL WHERE token = %(token)s AND price_usd > 0",
            parameters={"token": _liquidity_token(token)},
        ).result_rows
        if rows and rows[0][0]:
            return float(rows[0][0])
    return 0.0
...
def _snapshot_rpc(rpc_url: str, ch, token: str, block: int, *, timeout: int, retries: int) -> dict[str, Any]:
    ...
    return {"symbol": symbol, "underlying": underlying, "total_assets_raw": int(total_assets or 0), "total_supply_raw": int(total_supply or 0), "share_assets_raw": int(share_assets or 0), "share_decimals": int(share_decimals or 18), "assets_per_share": float(share_assets or 0) / float(10 ** int(underlying_decimals or 18)), "price_usd": price, "supply_usd": supply_usd, "errors": []}
...
def _load_ftoken_meta(ch, to_block: int) -> dict[str, dict[str, Any]]:
    if not _table_exists(ch, "fluid_ftoken_timeseries"):
        return {}
    rows = ch.query("""
        SELECT
            lower(product_id) AS product_id,
            argMax(symbol, tuple(block_number, inserted_at)) AS symbol,
            argMax(underlying, tuple(block_number, inserted_at)) AS underlying,
            argMax(price_usd, tuple(block_number, inserted_at)) AS price_usd
        FROM fluid_ftoken_timeseries FINAL
        WHERE block_number <= %(to_block)s
        GROUP BY product_id
    """, parameters={"to_block": int(to_block)}).result_rows
    return {
        normalize_address(token): {
            "symbol": str(symbol or ""),
            "underlying": normalize_address(underlying) if underlying else "",
            "price_usd": float(price_usd or 0.0),
            "decimals": 18,
        }
        for token, symbol, underlying, price_usd in rows
    }
...
def _load_raw_events(ch, from_block: int, to_block: int) -> list[tuple]:
    ...
    return ch.query("""
        ...
        FROM fluid_product_raw_events FINAL
        WHERE product_type = 'FTOKEN' AND block_number >= %(from_block)s AND block_number <= %(to_block)s
        ORDER BY block_number ASC, log_index ASC, tx_hash ASC, contract ASC, topic0 ASC
        """, parameters={"from_block": int(from_block), "to_block": int(to_block)}).result_rows
...
def replay(args, ch=None) -> int:
    ...
    try:
        ...
        for idx, ((token, block), state) in enumerate(sorted(points.items(), key=lambda item: (item[0][1], item[0][0]))):
            ...
            state_status = "DRIFT" if diff else "EVENT_REPLAY_INDEXED"
            provenance = {
        ...
        print(json.dumps({"status": "OK" if not drift else "DRIFT", "dryRun": bool(args.dry_run), "replayMode": "event_only", "fromBlock": from_block, "toBlock": to_block, "rawEvents": len(events), "liquidityIndexTokens": len(liquidity_prices), "fTokensWithEvents": len(states), "snapshotRows": len(rows), "writtenRows": written, "supplyDriftCount": len(drift), "sampleDrift": drift[:10]}, indent=2, sort_keys=True))
...
def _relative_diff_int(indexed: int, rpc: int) -> tuple[str, float, bool]:
    abs_diff = abs(int(indexed) - int(rpc))
    rel_diff = abs_diff / max(abs(int(rpc)), 1)
    return str(abs_diff), float(rel_diff), abs_diff != 0


def _relative_diff_str(indexed: str, rpc: str) -> tuple[str, float, bool]:
    mismatch = normalize_address(indexed) != normalize_address(rpc)
    return ("1" if mismatch else "0"), (1.0 if mismatch else 0.0), mismatch


def _append_anchor_diff(diffs: list[dict[str, Any]], target: str, entity_id: str, field: str, indexed: Any, rpc: Any, abs_diff: str, rel_diff: float) -> None:
    diffs.append({
        "target": target,
        "entity_id": entity_id,
        "field": field,
...
def anchor(args, ch=None) -> int:
    ...
    try:
        ...
        for key in missing[: int(getattr(args, "max_diff_rows", 500) or 500)]:
            _append_anchor_diff(diffs, target, f"log:{key[0]}:{key[1]}:{key[2]}", "raw_log_missing", "0", "1", "1", 1.0)
        for key in extra[: int(getattr(args, "max_diff_rows", 500) or 500)]:
            _append_anchor_diff(diffs, target, f"log:{key[0]}:{key[1]}:{key[2]}", "raw_log_extra", "1", "0", "1", 1.0)
        events = _load_raw_events(ch, 0, to_block)
        latest_ts: dict[str, dt.datetime] = {}
        for block, ts, contract, *_rest in events:
            latest_ts[str(contract).lower()] = ts
        meta = _load_ftoken_meta(ch, to_block)
        liquidity_prices = _load_liquidity_exchange_prices(
        ...
        for token in tokens:
            ...
            try:
                state = states.get(token.lower(), ReplayState())
                timestamp = latest_ts.get(token.lower()) or dt.datetime.now(dt.UTC).replace(tzinfo=None)
                event_snap = _snapshot_event_only(ch, token, to_block, timestamp, state, meta, liquidity_prices)
                rpc_snap = _snapshot_rpc(rpc_url, ch, token, to_block, timeout=timeout, retries=retries)
                share_decimals = int(rpc_snap.get("share_decimals", 18) or 18)
                event_share_assets_raw = (10 ** share_decimals) * int(event_snap.get("token_exchange_price", 10**12) or 10**12) // 10**12
                comparisons = [
                    ("total_supply_raw", int(state.supply_raw), int(rpc_snap["total_supply_raw"]), "int"),
                    ("total_assets_raw", int(event_snap["total_assets_raw"]), int(rpc_snap["total_assets_raw"]), "int"),
                    ("assets_per_share", int(event_share_assets_raw), int(rpc_snap.get("share_assets_raw", 0)), "int"),
                    ("underlying", event_snap.get("underlying", ""), rpc_snap.get("underlying", ""), "str"),
                ]
                checked += 1
                ...
                for field, indexed_value, rpc_value, kind in comparisons:
                    checked_fields += 1
                    if kind == "int":
                        abs_diff, rel_diff, mismatch = _relative_diff_int(int(indexed_value), int(rpc_value))
                        max_abs = max(max_abs, int(abs_diff))
                        max_rel = max(max_rel, rel_diff)
                    else:
                        abs_diff, rel_diff, mismatch = _relative_diff_str(str(indexed_value or ""), str(rpc_value or ""))
                        max_abs = max(max_abs, int(abs_diff))
                        max_rel = max(max_rel, rel_diff)
                    if mismatch:
                        drifted_fields += 1
                        entity_drifted = True
                        _append_anchor_diff(diffs, target, token, field, indexed_value, rpc_value, abs_diff, rel_diff)
                if entity_drifted:
                    drifted_entities.add(token)
            except Exception as exc:
                errors.append({"token": token, "error": str(exc)[:240]})
        if missing or extra:
            drifted_entities.add("raw_logs")
            drifted_fields += len(missing) + len(extra)
        ...
        payload = {
            ...
            "driftedEntities": len(drifted_entities),
        ...
        print(json.dumps(payload, indent=2, sort_keys=True))
        return 1 if status != "OK" and getattr(args, "fail_on_drift", False) else 0
    finally:
        if owned:
            ch.close()
