...
MASK_64 = (1 << 64) - 1
REPLAY_LOCK_OWNER = f"fluid-replay-{uuid.uuid4()}"
...
def _count_if_exists(ch, table: str, where_clause: str) -> int:
    try:
        return int(ch.query(f"SELECT count() FROM {table} WHERE {where_clause}").result_rows[0][0] or 0)
    except Exception:
        return 0


def _ensure_replay_lock_table(ch) -> None:
    ch.command(
        """
        CREATE TABLE IF NOT EXISTS fluid_replay_locks (
            lock_name String,
            owner String,
...
def _acquire_replay_lock(ch, owner: str, ttl_sec: int = 7200) -> None:
    _ensure_replay_lock_table(ch)
    active = ch.query(
        """
        SELECT owner, expires_at
        FROM fluid_replay_locks FINAL
        WHERE lock_name = 'fluid-replay' AND released = 0 AND expires_at > now()
        LIMIT 1
        """
    ).result_rows
    if active:
        raise RuntimeError(f"Fluid replay lock held by {active[0][0]} until {active[0][1]}")
    ch.insert(
        "fluid_replay_locks",
        [["fluid-replay", owner, dt.datetime.utcnow().replace(microsecond=0), dt.datetime.utcnow().replace(microsecond=0) + dt.timedelta(seconds=int(ttl_sec)), 0]],
        column_names=["lock_name", "owner", "acquired_at", "expires_at", "released"],
    )


def _release_replay_lock(ch, owner: str) -> None:
    try:
        ch.insert(
            "fluid_replay_locks",
            [["fluid-replay", owner, dt.datetime.utcnow().replace(microsecond=0), dt.datetime.utcnow().replace(microsecond=0), 1]],
            column_names=["lock_name", "owner", "acquired_at", "expires_at", "released"],
        )
    except Exception:
        return
...
def replay_fluid_from_raw(ch, *, from_block: int, to_block: int, batch_blocks: int = 50_000, dry_run: bool = False) -> dict[str, Any]:
    ...
    require_lock = os.getenv("FLUID_REPLAY_REQUIRE_LOCK", "true").strip().lower() not in {"0", "false", "no"}
    warmup_blocks = max(0, int(os.getenv("FLUID_REPLAY_WARMUP_BLOCKS", "250000") or "250000"))
    lock_owner = REPLAY_LOCK_OWNER
    range_info = ch.query(
        """
    ...
    cleanup_counts = {
        "fluid_reserve_metrics": _count_if_exists(ch, "fluid_reserve_metrics", f"timestamp >= '{start}' AND timestamp <= '{end}'"),
        "fluid_timeseries": _count_if_exists(ch, "fluid_timeseries", f"protocol = '{FLUID_MARKET}' AND timestamp >= '{start}' AND timestamp <= '{end}'"),
        "market_timeseries": _count_if_exists(ch, "market_timeseries", f"protocol = '{FLUID_MARKET}' AND timestamp >= '{start}' AND timestamp <= '{end}'"),
        "api_market_latest": _count_if_exists(ch, "api_market_latest", f"protocol = '{FLUID_MARKET}'"),
    }
    replay_start = max(int(FLUID_GENESIS_BLOCK), int(from_block) - warmup_blocks)
    if dry_run:
        return {
    ...
    if require_lock:
        _acquire_replay_lock(ch, lock_owner)
    try:
        ...
        if require_lock:
            _release_replay_lock(ch, lock_owner)
