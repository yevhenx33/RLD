...
@strawberry.type
class MorphoAllocationPoint:
    timestamp: int
    vault_address: str = strawberry.field(name="vaultAddress")
    vault_name: str = strawberry.field(name="vaultName")
    supplied_usd: float = strawberry.field(name="suppliedUsd")
    allocation_share: float = strawberry.field(name="allocationShare")


@strawberry.type
class MorphoAllocationVault:
    """Compact vault metadata for columnar allocation payloads."""
    id: int
    address: str
    name: str
    curator: str = ""
...
@strawberry.type
class MorphoAllocationColumnar:
    """Columnar (chart-shaped) allocation data.

    Instead of N*V rows with repeated vault addresses and names, we send:
    - timestamps: shared time axis (length T)
    - vaults: vault metadata (length V)
    - supplied_usd: 2D matrix flattened row-major [V][T] (length V*T)

    This reduces JSON size by ~4x for typical allocation histories and
    pairs extremely well with gzip compression.
    """
    timestamps: list[int]
    vaults: list[MorphoAllocationVault]
    supplied_usd: list[list[float]] = strawberry.field(name="suppliedUsd")
...
class LendingDataChartPoint:
    ...
    spark_tvl: float = strawberry.field(name="sparkTvl", default=0.0)
...
@strawberry.type
class ProtocolOverviewPagePayload:
    freshness: AnalyticsFreshness
    stats: ProtocolMarketsStats
    rows: list[ProtocolMarketRow]
    vaults: list[MetaMorphoVault]
    fluid_products: list[FluidProductSnapshot] = strawberry.field(name="fluidProducts")
    apy_history: list[ProtocolApyPoint] = strawberry.field(name="apyHistory")
    asset_apy_history: list[ProtocolAssetApyPoint] = strawberry.field(name="assetApyHistory")
    alluvial_flows: list[LendingFlowAlluvialLink] = strawberry.field(name="alluvialFlows")
    channel_flows: list[MorphoCuratorFlowLink] = strawberry.field(name="channelFlows")
    curator_allocation: list[MorphoCuratorAllocationPoint] = strawberry.field(name="curatorAllocation")
    fluid_vault_composition: list[FluidVaultCompositionPoint] = strawberry.field(name="fluidVaultComposition")
...
def _query_aave_event_flow_timeseries(
    ch,
    entity_id: str,
    resolution: str,
    limit: int,
    ...
    safe_limit = _safe_limit(limit)
    raw_table = "spark_events" if protocol == SPARK_MARKET else "aave_events"
    bucket_seconds = _bucket_seconds(resolution)
    now_dt = datetime.now(timezone.utc).replace(tzinfo=None, microsecond=0)
    window_start = now_dt - timedelta(seconds=(safe_limit + 2) * bucket_seconds)
    ...
    FROM (
        ...
        WHERE protocol IN ({protocol_names})
          AND (
            ...
            OR protocol = '{_escape_sql_string(SPARK_MARKET)}'
            OR supply_usd >= 1000
            OR borrow_usd >= 1000
          )
          AND NOT (
...
def _fluid_coverage_snapshot(ch) -> dict[str, object]:
    try:
        ...
        canonical_rows = 0
        product_status: list[dict[str, object]] = []
        ...
        product_raw_head = _query_int(ch, "SELECT max(block_number) FROM fluid_product_raw_events")
        ftoken_replay_head = _query_int(ch, "SELECT max(block_number) FROM fluid_ftoken_timeseries")
        vault_replay_rows = _query_int(ch, "SELECT count() FROM fluid_vault_timeseries FINAL")
        ...
        try:
            validation_rows = ch.query("""
                SELECT target, status, checked_entities, drifted_fields, max_rel_diff, anchor_block, toUnixTimestamp(finished_at)
                FROM fluid_anchor_runs FINAL
            ...
            validation_rows = []
        if validation_rows:
            target, status, checked_entities, drifted_fields, max_rel_diff, anchor_block, finished_at = validation_rows[0]
            validation = {
                "target": str(target),
                "status": str(status),
                "checkedCount": int(checked_entities or 0),
                "mismatchCount": int(drifted_fields or 0),
                "maxRelativeSupplyDiff": float(max_rel_diff or 0.0),
                "maxRelativeBorrowDiff": 0.0,
                "anchorBlock": int(anchor_block or 0),
                "finishedAt": int(finished_at or 0),
            }
        else:
            validation = {"status": "NEVER_RUN", "checkedCount": 0, "mismatchCount": 0, "maxRelativeSupplyDiff": 0.0, "maxRelativeBorrowDiff": 0.0, "finishedAt": 0}
        return {
            ...
            "productRawHead": int(product_raw_head or 0),
            "fTokenReplayHead": int(ftoken_replay_head or 0),
            "vaultReplayRows": int(vault_replay_rows or 0),
            "processedHead": int(processed_head or 0),
            "lagBlocks": max(0, int(raw_head or 0) - int(processed_head or 0)),
            "tvlPolicy": "Fluid direct-RPC product snapshots were removed; fToken/vault product surfaces are event-replay derived.",
...
def _build_lending_data_page_payload(
    ...
    for row in markets:
        ...
        collateral_sym = str(row.collateral_symbol) if row.collateral_symbol else None
    ...
    usdc_markets = [
        row for row in normalized_markets if str(row.symbol or "").upper() == "USDC"
    ]
    usdc_supply = sum(row.supply_usd for row in usdc_markets)
    usdc_borrow = sum(row.borrow_usd for row in usdc_markets)
    weighted_supply = sum(row.supply_apy * row.supply_usd for row in usdc_markets)
    weighted_borrow = sum(row.borrow_apy * row.borrow_usd for row in usdc_markets)
    stats = LendingDataStats(
        ...
        average_supply_apy=weighted_supply / usdc_supply if usdc_supply > 0 else 0.0,
        average_borrow_apy=weighted_borrow / usdc_borrow if usdc_borrow > 0 else 0.0,
    ...
    for row in tvl_history:
        ...
        spark_tvl = _finite_non_negative(row.spark)
    ...
    for row in apy_history:
        ...
        if row.sofr_rate is not None:
            existing.sofr_rate = _finite_non_negative(row.sofr_rate) * 100.0
...
def _query_morpho_allocation_timeseries(
    ch,
    market_id: str,
    top_n: int = 15,
    resolution: str = "1D",
    limit: int = MORPHO_ALLOCATION_DEFAULT_LIMIT,
) -> list[MorphoAllocationPoint]:
    """Return bounded allocation buckets for a single Morpho Blue market.

    Joins `metamorpho_vault_allocations` with `metamorpho_vault_registry` to
    resolve vault names. Returns the top N vaults by average supplied USD,
    aggregated into the requested backend bucket so the browser never receives
    raw allocation history.
    """
    normalized_market_id = _normalize_entity_id(market_id)
    if not normalized_market_id:
        return []

    safe_limit = _safe_limit(limit)
    safe_top_n = _safe_limit(top_n)
    bucket_seconds = _bucket_seconds(resolution)
    now_dt = datetime.now(timezone.utc).replace(tzinfo=None, microsecond=0)
    window_start = now_dt - timedelta(seconds=(safe_limit + 2) * bucket_seconds)
    ...
    WITH top_vaults AS (
        ...
        FROM metamorpho_vault_allocations
    ...
    FROM (
        ...
        WHERE a.market_id = %(market_id)s
          AND a.timestamp >= %(start_ts)s
          AND a.timestamp <= %(end_ts)s
          AND a.vault_address IN (SELECT vault_address FROM top_vaults)
        GROUP BY bucket_ts, a.vault_address
    )
    ORDER BY ts, vault_address
    """
    rows = ch.query(
        sql,
        parameters={
    ...
    # Build both row-based (legacy) and columnar (optimized) representations
    ...
    for ts, vault_address, _vault_name, _vault_curator, supplied_usd in rows:
        ...
        vi = vault_set[addr][0]
        ti = ts_index[ts_val]
        supplied_matrix[vi][ti] = float(supplied_usd or 0.0)
...
_lending_flow_cache: dict[int, tuple[float, list[LendingFlowAlluvialLink]]] = {}
...
def _query_lending_flow_alluvial(ch, window_days: int = 30) -> list[LendingFlowAlluvialLink]:
    ...
    with _lending_flow_cache_lock:
        cached = _lending_flow_cache.get(safe_days)
    ...
    try:
        ...
        spark_market_totals: dict[str, dict[str, float | str]] = {}
        spark_rows = ch.query(
            f"""
            SELECT
                day,
                lower(entity_id) AS entity_id,
                sum(supply_inflow_raw) AS supply_in_raw,
                sum(supply_outflow_raw) AS supply_out_raw,
                sum(borrow_inflow_raw) AS borrow_in_raw,
                sum(borrow_outflow_raw) AS borrow_out_raw
            FROM (
                SELECT
                    toStartOfDay(block_timestamp) AS day,
                    lower(concat('0x', substring(ifNull(topic1, ''), 27))) AS entity_id,
                ...
                WHERE block_timestamp >= %(end)s - INTERVAL {safe_days} DAY
                  ...
                  AND event_name IN ('Supply', 'Withdraw', 'Borrow', 'Repay', 'MintedToTreasury')
                ...
                WHERE block_timestamp >= %(end)s - INTERVAL {safe_days} DAY
                  AND block_timestamp <= %(end)s
                  AND event_name = 'LiquidationCall'
        ...
        for day, entity_id, s_in, s_out, b_in, b_out in spark_rows:
            symbol, decimals = _token_meta_for_entity(entity_id)
            price = spark_prices.get((day, str(entity_id).lower()), 0.0)
            denom = float(10 ** decimals)
            slot = spark_market_totals.setdefault(
                str(entity_id).lower(),
                {"symbol": symbol, "inflow_usd": 0.0, "outflow_usd": 0.0},
            )
            slot["inflow_usd"] = float(slot["inflow_usd"]) + (float(s_in or 0) + float(b_out or 0)) / denom * price
            slot["outflow_usd"] = float(slot["outflow_usd"]) + (float(s_out or 0) + float(b_in or 0)) / denom * price
        for slot in spark_market_totals.values():
            add_market_net("Spark", str(slot["symbol"]), float(slot["inflow_usd"]), float(slot["outflow_usd"]))
    ...
    links = links[:96]
    with _lending_flow_cache_lock:
        _lending_flow_cache[safe_days] = (time.time(), links)
    return links
...
def _query_lending_data_page(ch, display_in: str, flow_window_days: int = 30) -> LendingDataPagePayload:
    return _build_lending_data_page_payload(
        ...
        _query_usdc_lending_apy_history(ch, "1W", 5000),
        _query_lending_unique_users(ch),
        _query_lending_flow_alluvial(ch, flow_window_days),
    )
...
def _query_protocol_overview_page(
    ch,
    protocol: str,
    flow_window_days: int = 7,
    timeseries_limit: int = 5000,
    asset_symbols: Optional[list[str]] = None,
    max_borrow_apy: Optional[float] = None,
) -> ProtocolOverviewPagePayload:
    protocol_key = str(protocol or AAVE_MARKET).upper()
    symbols = asset_symbols or ["USDC", "USDT"]
    safe_limit = _api_page_size(timeseries_limit)
    safe_flow_days = max(1, min(90, int(flow_window_days or 7)))

    if protocol_key == COMPOUND_V3_MARKET:
        compound = _query_compound_v3_protocol_page(
            ch,
            safe_flow_days,
            safe_limit,
            symbols or ["USDC", "WETH"],
        )
        return ProtocolOverviewPagePayload(
            freshness=compound.freshness,
            stats=compound.stats,
            rows=compound.rows,
            vaults=[],
            fluid_products=[],
            apy_history=compound.apy_history,
            asset_apy_history=compound.asset_apy_history,
            alluvial_flows=compound.alluvial_flows,
            channel_flows=[],
            curator_allocation=[],
            fluid_vault_composition=[],
        )

    markets_page = _query_protocol_markets_page(ch, protocol_key, max_borrow_apy)
    channel_flows: list[MorphoCuratorFlowLink] = []
    curator_allocation: list[MorphoCuratorAllocationPoint] = []
    vaults: list[MetaMorphoVault] = []
    fluid_products: list[FluidProductSnapshot] = []
    fluid_vault_composition: list[FluidVaultCompositionPoint] = []

    if protocol_key == MORPHO_MARKET:
        vaults = _query_metamorpho_vaults(None, 2000)
        channel_flows = _query_morpho_curator_flows(ch, safe_flow_days, 10, max_borrow_apy)
        curator_allocation = _query_morpho_curator_allocation_history(
            ch,
            "1D",
            safe_limit,
            15,
    ...
    elif protocol_key == "EULER_MARKET":
        channel_flows = _query_euler_channel_flows(ch, safe_flow_days, 10, max_borrow_apy)
    elif protocol_key == FLUID_MARKET:
        fluid_products = [
            *_query_fluid_product_snapshots("FTOKEN", None, 2000),
            *_query_fluid_product_snapshots("VAULT", None, 2000),
        ]
        fluid_vault_composition = _query_fluid_vault_composition_history("1D", 50000)
    ...
    return ProtocolOverviewPagePayload(
        ...
        apy_history=_query_protocol_apy_history(ch, protocol_key, "1D", safe_limit, max_borrow_apy),
        asset_apy_history=_query_protocol_asset_apy_history(
            ch,
            protocol_key,
            symbols,
            "1D",
        ...
        alluvial_flows=_query_lending_flow_alluvial(ch, safe_flow_days),
        channel_flows=channel_flows,
        curator_allocation=curator_allocation,
        fluid_vault_composition=fluid_vault_composition,
    )


# ---------------------------------------------------------------------------
# In-process TTL cache for page-level payloads
# ---------------------------------------------------------------------------
_MARKET_PAGE_CACHE_TTL_SEC = 60
_market_page_cache: dict[str, tuple[float, LendingPoolPagePayload]] = {}
_market_page_cache_lock = threading.Lock()


def _market_page_cache_key(
    protocol: str, entity_id: str, ts_lim: int, flow_lim: int, alloc_lim: int
) -> str:
    return f"{protocol}|{entity_id}|{ts_lim}|{flow_lim}|{alloc_lim}"


def _market_page_cache_get(key: str) -> Optional[LendingPoolPagePayload]:
    with _market_page_cache_lock:
        entry = _market_page_cache.get(key)
        if entry is None:
            return None
        ts, payload = entry
        if time.time() - ts > _MARKET_PAGE_CACHE_TTL_SEC:
            del _market_page_cache[key]
            return None
        return payload


def _market_page_cache_set(key: str, payload: LendingPoolPagePayload) -> None:
    with _market_page_cache_lock:
        _market_page_cache[key] = (time.time(), payload)
        # Evict expired entries lazily (cap at 200 to avoid unbounded growth)
        if len(_market_page_cache) > 200:
            now = time.time()
            expired = [k for k, (t, _) in _market_page_cache.items()
                       if now - t > _MARKET_PAGE_CACHE_TTL_SEC]
            for k in expired:
                _market_page_cache.pop(k, None)
...
def _query_lending_pool_page(
    ...
    timeseries_limit: int,
    flow_limit: int,
    allocation_limit: int = MORPHO_ALLOCATION_DEFAULT_LIMIT,
) -> LendingPoolPagePayload:
    cache_key = _market_page_cache_key(
        protocol, entity_id, timeseries_limit, flow_limit, allocation_limit
    )
    cached = _market_page_cache_get(cache_key)
    if cached is not None:
        return cached

    # Run queries sequentially (clickhouse_connect prohibits concurrent
    # queries on the same session).  The HTTP response cache makes this
    # a non-issue: cache-miss runs ~500ms once, then all subsequent
    # requests within the 60s TTL return in <30ms from cached bytes.
    ...
    if protocol.upper() == "MORPHO_MARKET":
        alloc_rows, alloc_columnar = _query_morpho_allocation_timeseries(
            ch, entity_id, limit=allocation_limit
        )
    ...
    if protocol.upper() == "FLUID_MARKET":
        try:
            escaped_entity = _escape_sql_string(entity_id.lower())
            vault_rows = ch.query(f"""
                SELECT vault_id, symbol, supply_usd, borrow_usd, supply_apy, borrow_apy
                FROM fluid_vault_timeseries FINAL
                WHERE debt_token = '{escaped_entity}' AND symbol != ''
            ...
            if vault_rows:
                payload.vault_breakdown = [
                    FluidVaultRow(
                        ...
                        ltv=0.0,
                        supply_usd=float(row[2] or 0),
                        borrow_usd=float(row[3] or 0),
                        supply_apy=float(row[4] or 0),
                        borrow_apy=float(row[5] or 0),
                    )
...
def _query_fluid_vault_page(
    ...
    latest_rows = ch.query(f"""
        SELECT symbol, collateral_token, debt_token, supply_usd, borrow_usd,
               supply_apy, borrow_apy, utilization
        FROM fluid_vault_timeseries FINAL
        WHERE vault_id = '{escaped}' AND symbol != ''
        ORDER BY timestamp DESC, inserted_at DESC
        LIMIT 1
    """).result_rows
    market = None
    if latest_rows:
        ...
        market = LendingPoolMarketRow(
            protocol="FLUID_MARKET",
            entity_id=escaped,
            symbol=symbol,
            name=f"Fluid Vault {symbol}",
            supply_usd=float(row[3] or 0),
            borrow_usd=float(row[4] or 0),
            supply_apy=float(row[5] or 0) * 100.0,
            borrow_apy=float(row[6] or 0) * 100.0,
            utilization=float(row[7] or 0) * 100.0,
    ...
    while cursor <= max_week:
        ...
        points.append(
            ProtocolTvlPoint(
                ...
                spark=totals_by_protocol.get("SPARK", 0.0),
    ...
    for point, divisor in zip(points, weekly_prices):
        ...
        converted.append(
            ProtocolTvlPoint(
                ...
                spark=float(point.spark / divisor),
...
def _query_usdc_lending_apy_history(
    ch, resolution: str, limit: int
) -> list[ProtocolApyPoint]:
    safe_limit = _safe_limit(limit)
    protocol_names = ", ".join(
        f"'{_escape_sql_string(protocol)}'" for protocol in LENDING_DATA_MARKET_PROTOCOLS
    )
    time_expr = _time_bucket_expr(resolution, "timestamp")
    rows = ch.query(
        f"""
        WITH
            lending AS (
                SELECT
                    ...
                    if(
                        sum(supply_usd) > 0,
                        sum(bucket_supply_apy * supply_usd) / sum(supply_usd),
                        0.0
                    ) AS average_supply_apy,
                    if(
                        sum(borrow_usd) > 0,
                        sum(bucket_borrow_apy * borrow_usd) / sum(borrow_usd),
                        0.0
                    ) AS average_borrow_apy
                FROM (
                    SELECT
        ...
        SELECT
            toUnixTimestamp(lending.bucket_ts) AS bucket_ts,
            lending.average_supply_apy,
            lending.average_borrow_apy,
            sofr.sofr_rate
        FROM (
            SELECT
                1 AS join_key,
        ...
        ON lending.join_key = sofr.join_key AND lending.bucket_ts >= sofr.sofr_ts
        ORDER BY bucket_ts DESC
        LIMIT {safe_limit}
        """
    ).result_rows
    points = [
        ProtocolApyPoint(
            timestamp=int(row[0]),
            average_supply_apy=_finite_non_negative(row[1]),
            average_borrow_apy=_finite_non_negative(row[2]),
            sofr_rate=_finite_non_negative(row[3]) if len(row) > 3 and row[3] is not None else None,
        )
        for row in rows
    ]
    points.reverse()
...
def _query_morpho_event_flow_timeseries(
    ...
    """Compute gross supply/borrow inflow+outflow from individual Morpho Blue events.

    Joins ``morpho_market_events`` (Supply/Withdraw/Borrow/Repay) with
    ``morpho_market_params`` (loan_decimals) and ``morpho_market_metrics``
    (daily loan_price_usd) so every event is converted to USD.
    """
    safe_limit = _safe_limit(limit)
    trunc = {
        "1H": "toStartOfHour",
        "4H": "toStartOfInterval(e.timestamp, INTERVAL 4 HOUR)",
        "1W": "toStartOfWeek",
    }.get(resolution)
    if trunc and not trunc.startswith("toStartOf"):
        ...
        bucket_expr = f"{bucket_fn}(e.timestamp)"
    ...
    WITH daily_prices AS (
        SELECT
            ...
            argMax(loan_price_usd, timestamp) AS price
        FROM morpho_market_metrics
        WHERE market_id = %(mid)s AND loan_price_usd > 0
        GROUP BY day
    ),
    params AS (
        SELECT argMax(loan_decimals, updated_at) AS decimals
        FROM morpho_market_params
    ...
    LEFT JOIN daily_prices AS dp ON toStartOfDay(e.timestamp) = dp.day
    WHERE e.market_id = %(mid)s
      AND e.event_name IN ('Supply', 'Withdraw', 'Borrow', 'Repay')
    GROUP BY ts
    ORDER BY ts
    LIMIT %(lim)s
    """
    rows = ch.query(
        sql, parameters={"mid": entity_id, "lim": safe_limit}
    ).result_rows
    if not rows:
        return []
...
def _query_market_flow_timeseries(
    ...
    if protocol:
        ...
        elif protocol.upper() == "SPARK_MARKET":
            return _query_aave_event_flow_timeseries(
                ch, entity_id, resolution, limit, SPARK_MARKET
            )
...
def _query_pendle_eth_assets(
    ...
    conditions = ["p.chain_id = 1"]
    type_filter = _pendle_type_filter(asset_types)
    if type_filter:
        conditions.append(type_filter.replace(" AND ", "", 1).replace("asset_type", "p.asset_type"))
    if active_only:
...
def _query_pendle_eth_latest_prices(
    ...
    if addresses:
        ...
        if normalized:
            ...
            conditions.append(f"p.asset_address IN ({values})")
    sql = f"""
        SELECT
            p.asset_address,
            p.chain_id,
            p.asset_type,
            p.symbol,
            if(p.expiry > toDateTime(0), p.expiry, a.expiry) AS expiry,
            p.price_usd,
            p.source_timestamp
        FROM pendle_eth_price_latest AS p FINAL
        LEFT JOIN pendle_eth_assets AS a FINAL ON p.asset_address = a.asset_address
        WHERE {' AND '.join(conditions)}
        ORDER BY p.source_timestamp DESC, p.symbol ASC, p.asset_address ASC
        LIMIT {_safe_limit(limit)}
    """
    rows = ch.query(sql).result_rows
    return [
        PendleLatestPrice(
            ...
            asset_type=str(row[2]),
            symbol=str(row[3]),
            expiry=_timestamp(row[4]),
            price_usd=float(row[5]),
...
def _query_pendle_eth_price_history(
    ...
    conditions = [
        "h.chain_id = 1",
        f"h.asset_address = '{_escape_sql_string(asset_address)}'",
        f"h.time_frame = '{frame}'",
    ]
    if start_ts is not None:
        conditions.append(f"h.timestamp >= toDateTime({max(0, int(start_ts))})")
    if end_ts is not None:
        conditions.append(f"h.timestamp <= toDateTime({max(0, int(end_ts))})")
    sql = f"""
        SELECT
            h.timestamp,
            if(h.expiry > toDateTime(0), h.expiry, a.expiry) AS expiry,
            h.open,
            h.high,
            h.low,
            h.close,
            h.volume
        FROM pendle_eth_price_ohlcv AS h FINAL
        LEFT JOIN pendle_eth_assets AS a FINAL ON h.asset_address = a.asset_address
        WHERE {' AND '.join(conditions)}
...
def _fluid_ts_to_unix(value) -> int:
    if isinstance(value, datetime):
        return int(value.replace(tzinfo=timezone.utc).timestamp()) if value.tzinfo is None else int(value.timestamp())
    return int(value or 0)


def _fluid_product_from_event_row(
    product_type: str,
    product_id: str,
    timestamp,
    block_number: int,
    ...
    ltv: float = 0.0,
    state_status: str = "EVENT_REPLAY",
    provenance: str = "",
) -> FluidProductSnapshot:
    supply = _finite_non_negative(supply_usd)
    borrow = _finite_non_negative(borrow_usd)
    return FluidProductSnapshot(
        chain_id=1,
        product_type=product_type,
        product_id=str(product_id or "").lower(),
        timestamp=_fluid_ts_to_unix(timestamp),
        block_number=int(block_number or 0),
        symbol=str(symbol or product_type),
        underlying=str(underlying or ""),
        collateral_token=str(collateral_token or ""),
        debt_token=str(debt_token or ""),
        supply_usd=supply,
        borrow_usd=borrow,
        collateral_usd=supply,
        liquidity_usd=max(0.0, supply - borrow),
        volume_usd=0.0,
        fees_usd=0.0,
        supply_apy=float(supply_apy or 0.0),
        borrow_apy=float(borrow_apy or 0.0),
        utilization=float(utilization or 0.0),
        ltv=float(ltv or 0.0),
        liquidation_threshold=0.0,
        position_count=0,
        is_canonical_tvl=False,
        pricing_status="EVENT_REPLAY",
        oracle_status="EVENT_REPLAY_PRICING",
        snapshot_status=str(state_status or "EVENT_REPLAY"),
        provenance=provenance or '{"source":"event_replay"}',
        error="",
    )


def _query_fluid_product_snapshots(product_type: Optional[str] = None, product_id: Optional[str] = None, limit: int = 500) -> list[FluidProductSnapshot]:
    """Compatibility field backed by event-replay tables, not RPC snapshots."""
    ch = get_clickhouse_client()
    requested = str(product_type or "").upper() or None
    safe_limit = max(1, min(int(limit or 500), 5000))
    escaped_id = _escape_sql_string(str(product_id or "").lower())
    rows: list[FluidProductSnapshot] = []

    if requested in (None, "VAULT"):
        vault_filter = f"AND vault_id = '{escaped_id}'" if escaped_id else ""
        try:
            vault_rows = ch.query(f"""
                SELECT
                    vault_id,
                    ...
                    argMax(utilization, tuple(timestamp, inserted_at)) AS utilization
                FROM fluid_vault_timeseries FINAL
                WHERE notEmpty(fluid_vault_timeseries.symbol) {vault_filter}
                GROUP BY vault_id
                ORDER BY supply_usd + borrow_usd DESC
                LIMIT {safe_limit}
            """).result_rows
            rows.extend(
                _fluid_product_from_event_row(
                    "VAULT", row[0], row[1], 0, row[2], "",
                    row[3], row[4], row[5], row[6], row[7], row[8], row[9],
                    state_status="EVENT_REPLAY",
                    provenance='{"source":"fluid_vault_timeseries"}',
                )
                for row in vault_rows
            )
        except Exception as exc:
            logger.warning("Fluid vault event-product query failed: %s", exc)

    if requested in (None, "FTOKEN"):
        ftoken_filter = f"AND product_id = '{escaped_id}'" if escaped_id else ""
        try:
            ftoken_rows = ch.query(f"""
                SELECT
                    product_id,
                    max(timestamp) AS ts,
                    argMax(fluid_ftoken_timeseries.block_number, tuple(timestamp, fluid_ftoken_timeseries.block_number, inserted_at)) AS block_number,
                    argMax(symbol, tuple(timestamp, fluid_ftoken_timeseries.block_number, inserted_at)) AS symbol,
                    argMax(underlying, tuple(timestamp, fluid_ftoken_timeseries.block_number, inserted_at)) AS underlying,
                    argMax(supply_usd, tuple(timestamp, fluid_ftoken_timeseries.block_number, inserted_at)) AS supply_usd,
                    argMax(state_status, tuple(timestamp, fluid_ftoken_timeseries.block_number, inserted_at)) AS state_status,
                    argMax(provenance, tuple(timestamp, fluid_ftoken_timeseries.block_number, inserted_at)) AS provenance
                FROM fluid_ftoken_timeseries FINAL
                WHERE product_id != '' {ftoken_filter}
                GROUP BY product_id
                ORDER BY supply_usd DESC
                LIMIT {safe_limit}
            """).result_rows
            rows.extend(
                _fluid_product_from_event_row(
                    "FTOKEN", row[0], row[1], row[2], row[3], row[4],
                    "", "", row[5], 0.0, 0.0, 0.0, 0.0,
                    state_status=row[6] or "EVENT_REPLAY",
                    provenance=row[7] or '{"source":"fluid_ftoken_timeseries"}',
                )
                for row in ftoken_rows
            )
        except Exception as exc:
            logger.warning("Fluid fToken event-product query failed: %s", exc)

    rows.sort(key=lambda item: (item.supply_usd + item.borrow_usd), reverse=True)
...
def _query_fluid_product_snapshot_history(
    ...
    """Compatibility history backed by event-replay tables, not RPC snapshots."""
    ch = get_clickhouse_client()
    requested = str(product_type or "").upper() or None
    safe_limit = _safe_limit(limit)
    rows: list[FluidProductSnapshot] = []
    ...
    if requested in (None, "VAULT"):
        bucket_expr = _time_bucket_expr(resolution, "timestamp")
        try:
            vault_rows = ch.query(f"""
                SELECT
                    vault_id,
                    ...
                    argMax(utilization, tuple(timestamp, inserted_at)) AS utilization
                FROM fluid_vault_timeseries FINAL
                WHERE notEmpty(fluid_vault_timeseries.symbol)
                GROUP BY vault_id, bucket_ts
                ORDER BY bucket_ts DESC, supply_usd + borrow_usd DESC
                LIMIT {safe_limit}
            """).result_rows
            rows.extend(
                _fluid_product_from_event_row(
            ...
            logger.warning("Fluid vault event-history query failed: %s", exc)
    ...
    if requested in (None, "FTOKEN"):
        ...
        try:
            ftoken_rows = ch.query(f"""
                ...
                FROM fluid_ftoken_timeseries FINAL
                WHERE product_id != ''
                GROUP BY product_id, bucket_ts
                ORDER BY bucket_ts DESC, supply_usd DESC
                LIMIT {safe_limit}
            """).result_rows
            rows.extend(
                _fluid_product_from_event_row(
            ...
            logger.warning("Fluid fToken event-history query failed: %s", exc)

    rows.sort(key=lambda item: (item.timestamp, item.supply_usd + item.borrow_usd), reverse=True)
    return rows[:safe_limit]
...
def _query_fluid_vault_composition_history(
    ...
    bucket_expr = _time_bucket_expr(resolution, "timestamp")
    query = f"""
        WITH per_vault_bucket AS (
            SELECT
                {bucket_expr} AS bucket_ts,
                vault_id,
                argMax(symbol, tuple(timestamp, inserted_at)) AS symbol,
                argMax(supply_usd, tuple(timestamp, inserted_at)) AS collateral_usd,
                argMax(borrow_usd, tuple(timestamp, inserted_at)) AS debt_usd
            FROM fluid_vault_timeseries FINAL
            WHERE notEmpty(fluid_vault_timeseries.symbol)
    ...
    points = [
        ...
        for row in ch.query(query).result_rows
...
def _query_fluid_vault_page(
    ...
    """Build a vault-level detail page only from event-replay output."""
    escaped = _escape_sql_string(str(vault_id or "").lower())
    ts_rows = ch.query(f"""
        SELECT toUnixTimestamp(timestamp) AS ts,
               supply_usd, borrow_usd, supply_apy, borrow_apy, utilization
        FROM fluid_vault_timeseries FINAL
    ...
    if latest_rows:
        ...
        market = MarketDetail(
            ...
            collateral_symbol=col_sym,
            loan_asset=debt_sym,
            supply_usd=_finite_non_negative(row[3]),
            borrow_usd=_finite_non_negative(row[4]),
            supply_apy=float(row[5] or 0.0),
            borrow_apy=float(row[6] or 0.0),
            utilization=float(row[7] or 0.0),
            lltv_min=0.0,
            lltv_max=0.0,
            collateral_price_usd=None,
            oracle_support="event replay",
        )
...
class Query:
    ...
    def market_page(
        ...
        return _query_lending_pool_page(
            ...
            else MORPHO_ALLOCATION_DEFAULT_LIMIT,
        )
    ...
    @strawberry.field(name="protocolOverviewPage")
    def protocol_overview_page(
        self,
        protocol: str = AAVE_MARKET,
        flow_window_days: int = 7,
        timeseries_limit: int = 5000,
        ...
        ch = get_clickhouse_client()
        return _query_protocol_overview_page(
            ch,
            protocol,
            flow_window_days,
            _api_page_size(timeseries_limit),
            asset_symbols or ["USDC", "USDT"],
            max_borrow_apy,
        )
