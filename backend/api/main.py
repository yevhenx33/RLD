...
# Import indexer modules (skip in RATE_ONLY mode)
...
if not _RATE_ONLY:
    ...
    logging.info("📊 RATE_ONLY mode: skipping chain-dependent imports")
...
app.add_middleware(
    ...
    allowed_hosts=["localhost", "127.0.0.1", "0.0.0.0", "testserver", "rate-dashboard.onrender.com", "rld.fi", "www.rld.fi", "rates-indexer", "host.docker.internal"]
)
...
if not _RATE_ONLY:
    ...
    logging.info("📊 RATE_ONLY mode: skipping Web3 and ABI setup")
...
async def startup_event_indexers():
    ...
    # RATE_ONLY mode: skip chain-dependent indexers
    rate_only = os.getenv("RATE_ONLY", "").lower() in ("true", "1", "yes")
    if rate_only:
        logging.info("📊 Running in RATE_ONLY mode - skipping chain indexers")
