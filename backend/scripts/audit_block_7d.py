...
def audit_block_7d():
    print(f"🔍 AUDIT: Block-Level Data (Last 7 Days)")
    ...
    print(f"📅 Range: {datetime.fromtimestamp(seven_days_ago)} -> {datetime.fromtimestamp(now)}")
    ...
    if total_gaps == 0:
        print("\n✅ PASSED: No gaps > 15s in the last 7 days.")
    else:
        print(f"\n❌ FAILED: Found {total_gaps} gaps > 15s.")
        sys.exit(1)
