...
SYNC_INTERVAL = 10  # Sync to clean_rates.db every N seconds
...
def sync_clean_db():
    """Trigger sync to clean_rates.db."""
    script_dir = os.path.dirname(os.path.abspath(__file__))
    sync_script = os.path.join(script_dir, "sync_clean_db.py")
    
    if os.path.exists(sync_script):
        try:
            subprocess.run(["python3", sync_script], capture_output=True, timeout=60)
            logger.info("🔄 Synced to clean_rates.db")
        except Exception as e:
    ...
    while running:
        try:
            ...
            if last_indexed < current_block:
                ...
                # Sync to clean_rates.db periodically
