---
title: Rate-Level Perpetuals
description: Transforming volatile DeFi yield into a tradable, persistent derivative
---

# 02: Rate-Level Perpetuals (RLP)
...
![cdp_architecture.png](cdp_architecture.png)
...
3. **Liquidation Thresholds**: The Vault enforces over-collateralization parameters - if the debt value exceeds the liquidation threshold (e.g., 125% Collateralization Ratio) due to a severe rate spike, the collateral is auctioned to repay the RLP debt.

This structure creates a fungible asset tracking $P_{index}$ without requiring complex cash-funding machinery, allowing the perpetual to trade on external Automated Market Makers (AMMs).
...
![eth_usdc_cointegration.png](eth_usdc_cointegration.png)
...
3. **The Q4 Structural Break Resolution:** The base Q4 data displays a total decoupling (Correlation = 0.254, Lag-0 p-value = 0.4618), triggered by the Stream Finance \$93M liquidity crisis causing panic withdrawals and artificial rate inflation independent of ETH price action. However, shifting the Engle-Granger test by a 7-day lag restores mathematical significance (p=0.0436). This empirically proves that the extreme rate dislocation was fundamentally temporary, and the algorithmic gravity of the lending pool forcibly restored equilibrium exactly 7 days post-shock, offering a highly predictable statistical arbitrage window for RLP shorts.
